...
import { getHolidaysInYear, isHoliday } from "./index";

describe("日本の祝日", () => {
  it("固定祝日を判定できること", () => {
    // 元日
    expect(isHoliday(new Date(2025, 0, 1))).toBe("元日");
    // 建国記念の日
    expect(isHoliday(new Date(2025, 1, 11))).toBe("建国記念の日");
    // 令和の天皇誕生日 (2020~)
    expect(isHoliday(new Date(2025, 1, 23))).toBe("天皇誕生日");
  });

  it("過去の祝日（旧天皇誕生日など）を判定できること", () => {
    // 平成の天皇誕生日 (1989-2018): 12月23日
    expect(isHoliday(new Date(2018, 11, 23))).toBe("天皇誕生日");
    expect(isHoliday(new Date(2019, 11, 23))).toBeUndefined(); // 2019年はなし

    // 4月29日の変遷
    // 昭和の日 (2007~): 4月29日
    expect(isHoliday(new Date(2025, 3, 29))).toBe("昭和の日");
    // みどりの日 (1989-2006): 4月29日
    expect(isHoliday(new Date(2000, 3, 29))).toBe("みどりの日");
    // 昭和の天皇誕生日 (1949-1988): 4月29日
    expect(isHoliday(new Date(1988, 3, 29))).toBe("天皇誕生日");

    // 5月4日の変遷
    // みどりの日 (2007~): 5月4日
    expect(isHoliday(new Date(2025, 4, 4))).toBe("みどりの日");
    // 2006年以前は祝日としては存在しない（国民の休日としてはありうる）
    // 2006年5月4日は木曜日。5/3(憲法)と5/5(こども)の間なので国民の休日となるはず
    expect(isHoliday(new Date(2006, 4, 4))).toBe("国民の休日");
  });

  it("ハッピーマンデー制度の祝日を判定できること", () => {
    // 成人の日 (2000~): 1月の第2月曜日
    // 2025年1月1日は水曜日 -> 第2月曜: 13日
    expect(isHoliday(new Date(2025, 0, 13))).toBe("成人の日");

    // 旧成人の日 (1948-1999): 1月15日固定
    expect(isHoliday(new Date(1999, 0, 15))).toBe("成人の日");
    // 2000年以降の1月15日は成人の日ではない (偶然重なる場合を除くが、2000年1月15日は土曜日、第2月曜は10日)
    expect(isHoliday(new Date(2000, 0, 15))).toBeUndefined();
  });

  it("振替休日を判定できること", () => {
    // 2024年5月5日(日)はこどもの日
    // 5月6日(月)は振替休日になるはず
    expect(isHoliday(new Date(2024, 4, 5))).toBe("こどもの日");
    expect(isHoliday(new Date(2024, 4, 6))).toBe("振替休日");
  });

  it("春分・秋分の日を判定できること (既知の日付で検証)", () => {
    // 2024年 春分: 3月20日
    expect(isHoliday(new Date(2024, 2, 20))).toBe("春分の日");
    // 2024年 秋分: 9月22日
    expect(isHoliday(new Date(2024, 8, 22))).toBe("秋分の日");
  });
  ...
  it("国民の休日を判定できること", () => {
    ...
    // 5月3日: 憲法記念日
    // 5月2日は国民の休日
    expect(isHoliday(new Date(2019, 4, 2))).toBe("国民の休日");

    // シルバーウィーク 2015
    // 9月21日 (月): 敬老の日
    // 9月22日 (火): 国民の休日
    // 9月23日 (水): 秋分の日
    expect(isHoliday(new Date(2015, 8, 21))).toBe("敬老の日"); // 月
    expect(isHoliday(new Date(2015, 8, 23))).toBe("秋分の日"); // 水
    expect(isHoliday(new Date(2015, 8, 22))).toBe("国民の休日"); // 火
  });

  it("オリンピックによる山の日の移動を正しく処理できること", () => {
    // 2020: 8月10日
    expect(isHoliday(new Date(2020, 7, 10))).toBe("山の日");
    // 2021: 8月8日
    expect(isHoliday(new Date(2021, 7, 8))).toBe("山の日");
    // 2022: 8月11日 (通常)
    expect(isHoliday(new Date(2022, 7, 11))).toBe("山の日");
  });

  it("祝日でない場合は undefined を返すこと", () => {
    expect(isHoliday(new Date(2025, 0, 2))).toBeUndefined();
  });
